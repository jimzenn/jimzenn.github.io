...
More generally, we are given a sample space $\Omega$, a probability law $P$, suppose $B \subseteq \Omega, P(B) > 0, P(B) > 0.$
...
$P(\cap ^ n \_ {i = 1} A\_i) = P(A\_1) P(A\_2 | A\_1) P(A\_3 | A\_1, A\_2) ... P(A\_3 | A\_1, ..., A\_n - 1)$
...
{{% example name="" %}}
$A\_i = $ the first $i $ people have the same birthday.

$P(\cap^n\_{ i = 1 } A\_i) = 1 \cdot (1 - \frac{ 1 }{ 365 }) (1 - \frac{ 2 }{ 365 }) ... (1 - \frac{ n-1 }{ 365 })$

$1 - P(\cap^n\_{ i = 1 } A\_i) $ is the probability that at least two people have the same probability.
