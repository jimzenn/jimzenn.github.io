...
1. **(Time-homogeneity)** The probability $P(k, \tao) $ of $k $ arrivals is the same for all intervals of the same length $\tao $.
...
3. **(Small interval probabilities)** The probabilities $P(k, \tao) $ satisfy

$$\begin{align\*}
P(0, \tao) &= 1 - \lambda \tao + o(\tao), \br
P(1, \tao) &= \lambda \tao + o\_1(\tao), \br
P(k, \tao) &= o\_k(\tao),\text{ for } k = 2, 3, ...
\end{align\*}$$

Here, $o(\tao) $ and $o\_k(\tao) $ are functions of $\tao $ that satisfy

$$\limu{ \tao }{ o } \frac{ o(\tao)}{ \tao } = 0, \limu{ \tao }{ o } \frac{ o\_k(\tao)}{ \tao } = 0 $$
{{% /definition %}}
