...
Linear transformations (linear maps) and their basic properties, identity transformation, zero transformation, kernel (null space), range (image), nullity, rank, ...
...
If $\beta = \set{x\_1, x\_2, ..., x\_{n}}$ is a basis of $V\_1$, then $R(T) = \spa{T(x\_1), T(x\_2), ... T(x\_n)}$.
...
Let $\beta = \set{x\_1, x\_2, ..., x\_{k}}$ be a basis of $N(T)$,

Since $N(T) \leq V, \beta$ can be extended to a basis of $V$ , saying $\gamma  = \beta \cup \set{y\_1, y\_2, ..., y\_{n-k}}$, where $n = \dim V$.
...
Null space always contains at least one vector, namely, the null vector $0$ i.e. $0 \in N(T)$
